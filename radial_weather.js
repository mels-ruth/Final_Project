d3.csv("weather_severity_counts.csv").then(data => {
    ...
    const maxLogged = Math.log1p(d3.max(data, d => d.Proportion));
    ...
    const legendData = d3.range(0, 1.001, 0.01);
    ...
    legendsvg.selectAll("rect")
        ...
        .append("rect")
        .attr("x", 0)
        .attr("y", d => (1 - d) * legendHeight)  
        .attr("width", legendWidth)
        .attr("height", legendHeight / legendData.length)
        .attr("fill", d => color(d * maxLogged));

    const legendScale = d3.scaleLinear()
        .domain([d3.max(data, d => d.Proportion), 0])  
